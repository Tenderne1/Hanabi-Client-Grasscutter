...
public class HandlerEvtDoSkillSuccNotify extends PacketHandler {
    ...
    public void handle(GameSession session, byte[] header, byte[] payload) throws Exception {
        ...
        if (avatarSkillData != null && avatarSkillData.getCostElemVal() > 0) {
            session.getPlayer().setPlayerElementBurstInvincibleEndTime(TimeUnit.NANOSECONDS.toMillis(System.nanoTime()) - session.getPlayer().getSession().getLag() + TimeUnit.MILLISECONDS.toMillis(2000));
        }
